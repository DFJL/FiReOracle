...
export function findPdfPart(part: GmailPart): GmailPart | null {
...
export async function fetchPdfAttachment(msgId: string, attachmentId: string, accessToken: string): Promise<string | null> {
...
export function decodeEmailBody(part: GmailPart): string {
  if (part.body?.data) {

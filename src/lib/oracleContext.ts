...
import { inferCategory, displayCategory, isLoanPayment, SAVINGS_EXPENSE_GROUP } from '@/app/(dashboard)/resumen/categoryUtils'
import { buildRootCodeMap, cleanLifestyleOutliers } from '@/lib/lifestyleExpenses'

// Single source of truth for "what does Oracle know about the user" — used by
// both the web page (oracle/page.tsx) and the Telegram webhook, so the two
// channels can't drift into different context-building logic (the exact
// problem that made the web Oracle's category totals wrong before it was
// fixed to reuse cleanLifestyleOutliers).
...
export async function buildOracleContext(userId: string): Promise<string> {
  ...
  // Real trailing-12m average whenever there's spending history; the manual
  // config value is only a bootstrap fallback for a brand-new user with no
  // history yet. Kept in sync with /progreso.
  const targetExp  = avgLifestyleExpenses > 0 ? avgLifestyleExpenses : (fireConfig?.fire_target_monthly_exp ?? 0)
  ...
  const expensePatterns = topCats.map(([cat]) => {
    ...
    return { cat, avg, med, activeMonths, spikeMonths }
  }).filter(p => p.avg > 0)
  ...
  // ─────────────────────────── BUILD CONTEXT ──────────────────────────────────
  return `FECHA HOY: ${now.toISOString().slice(0, 10)}
